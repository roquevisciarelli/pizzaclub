document.addEventListener('DOMContentLoaded', () => {
    ...
    function showDashboard() {
        ...
        loadInitialData();
        setupSearch('category-search', 'categories-table-body');
    ...
    async function loadCategories() {
        try {
            ...
            document.querySelectorAll('#categories-table-body .btn-delete').forEach(btn => {
                btn.addEventListener('click', (e) => handleDeleteCategory(e.target.dataset.id));
            });
        } catch (error) {
    ...
    async function handleDeleteCategory(id) {
        if (confirm('¿Estás seguro de eliminar esta categoría? Esto también eliminará sus productos.')) {
            try {
                await fetchWithAuth('/api/admin/categorias/' + id, { method: 'DELETE' });
                loadCategories();
            } catch (error) {
    ...
    async function handleDeleteProduct(id) {
        if (confirm('¿Estás seguro de eliminar este producto?')) {
            try {
                await fetchWithAuth('/api/admin/productos/' + id, { method: 'DELETE' });
                loadProducts();
            } catch (error) {
    ...
    productForm.addEventListener('submit', async (e) => {
        ...
        const data = {
            ...
            categoriaId: parseInt(document.getElementById('product-category').value),
        ...
        input.addEventListener('input', () => {
            ...
            Array.from(tbody.rows).forEach(row => {
                const matches = Array.from(row.cells).some(cell =>
                    normalizeText(cell.textContent).includes(term)
                );
